...
//! - Green increases downward (V coordinate)
//! - Blue channel pulses over time
//!
//! Run with: `cargo run -p pixel_world --example uv_quad`

use bevy::prelude::*;
use pixel_world::{Blitter, Chunk, Rect, Rgba, create_texture, upload_surface};
...
struct UvQuadState {
    ...
    /// Position of the quad (top-left corner).
    pos: Vec2,
...
fn setup(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
    ...
    // Spawn sprite to display the chunk
    commands.spawn(Sprite {
        image: texture_handle.clone(),
        custom_size: Some(Vec2::new(CHUNK_SIZE as f32 * 2.0, CHUNK_SIZE as f32 * 2.0)),
        ..default()
    });
...
fn update_quad(mut state: ResMut<UvQuadState>, time: Res<Time<Fixed>>) {
    ...
    blitter.blit(rect, |_x, _y, u, v| {
        Rgba::new(
            ...
            (v * 255.0) as u8,     // Green increases down
            blue,                   // Blue pulses
            255,
        )
    });
