...
use crate::parallel::blitter::LockedChunks;
use crate::pixel::Pixel;

/// Returns the position to swap with, or None if pixel stays.
///
/// This is the parallel-safe version that reads from LockedChunks.
pub fn compute_swap(
    pos: WorldPos,
...
fn compute_powder_swap(
    ...
    // Try sliding diagonally (randomize direction based on position for determinism)
...
/// Reads a pixel from locked chunks.
fn get_pixel(chunks: &LockedChunks<'_>, pos: WorldPos) -> Option<Pixel> {
    let (chunk_pos, local) = pos.to_chunk_and_local();
    let guard = chunks.get(chunk_pos)?.lock().ok()?;
    Some(guard.pixels[(local.0 as u32, local.1 as u32)])
}
