...
use crate::coords::{ChunkPos, TilePos, WorldFragment, WorldPos, WorldRect, CHUNK_SIZE, POOL_SIZE, WINDOW_HEIGHT, WINDOW_WIDTH};
...
impl PixelWorld {
        ...
        F: Fn(WorldFragment) -> Option<Pixel> + Sync,
    {
        ...
        let dirty: Vec<_> = dirty_chunks.into_inner().unwrap_or_default().into_iter().collect();
        let dirty_tile_list: Vec<_> = dirty_tiles.into_inner().unwrap_or_default().into_iter().collect();
