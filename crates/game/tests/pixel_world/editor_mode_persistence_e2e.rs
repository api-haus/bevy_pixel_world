...
impl EditorModeTestHarness {
  ...
  fn run_until_handle_complete(&mut self, handle: &game::pixel_world::PersistenceHandle) {
  ...
  fn save(&mut self) -> game::pixel_world::PersistenceHandle {
  ...
  fn paint_pattern(&mut self, center: WorldPos, material: game::pixel_world::MaterialId) {
  ...
  fn get_material_at(&mut self, pos: WorldPos) -> Option<game::pixel_world::MaterialId> {
  ...
  fn verify_pattern_exists(
    &mut self,
    center: WorldPos,
    expected_material: game::pixel_world::MaterialId,
  ) -> bool {
    // Check center pixel
