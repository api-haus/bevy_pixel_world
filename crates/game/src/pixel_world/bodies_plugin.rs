...
use crate::pixel_world::world::persistence_systems::{
  save_pixel_bodies_on_chunk_unload, save_pixel_bodies_on_request,
};
use crate::pixel_world::world::streaming::{
...
///
/// Must be added after
/// [`PixelWorldPlugin`](crate::pixel_world::PixelWorldPlugin). Panics at
...
impl Plugin for PixelBodiesPlugin {
  fn build(&self, app: &mut App) {
    // Validate dependency
    assert!(
      app.is_plugin_added::<crate::pixel_world::PixelWorldPlugin>(),
    ...
    app.add_systems(
      ...
      draw_collision_gizmos
        .run_if(resource_exists::<crate::pixel_world::world::plugin::RenderingEnabled>),
    );
  }
}
