...
use crate::pixel_world::persistence::{
  PersistenceTasks, PixelBodyRecord, compression::compress_lz4, format::StorageType,
};
use crate::pixel_world::pixel_body::{LastBlitTransform, Persistable, PixelBody, PixelBodyId};
...
pub(crate) fn handle_clear_persistence(
  ...
  io_dispatcher.send(crate::pixel_world::persistence::IoCommand::DeleteSave);
}
...
///
/// Shared helper for save systems to avoid duplicating velocity extraction
/// logic.
pub(crate) fn create_body_record_blitted(
  ...
  velocities: &Query<Option<&bevy_rapier2d::prelude::Velocity>>,
...
fn save_matching_bodies(
  ...
  for (entity, body_id, body, _, blitted) in bodies.iter() {
    ...
    let Some(record) = create_body_record_blitted(entity, body_id, body, blitted, velocities)
    else {
      continue;
    };
...
fn dispatch_body_saves(tasks: &mut PersistenceTasks, io_dispatcher: &IoDispatcher) {
  for task in tasks.body_save_queue.drain(..) {
    ...
    io_dispatcher.send(crate::pixel_world::persistence::IoCommand::SaveBody {
      record_data: buf,
...
fn dispatch_body_removals(tasks: &mut PersistenceTasks, io_dispatcher: &IoDispatcher) {
  for task in tasks.body_remove_queue.drain(..) {
    io_dispatcher.send(crate::pixel_world::persistence::IoCommand::RemoveBody {
...
pub(crate) fn poll_save_task(
  ...
  #[cfg(not(target_family = "wasm"))] rendering: Option<
    Res<crate::pixel_world::world::plugin::RenderingEnabled>,
  >,
  #[cfg(not(target_family = "wasm"))] async_behavior: Option<
    Res<crate::pixel_world::world::plugin::AsyncTaskBehavior>,
  >,
) {
  #[cfg(not(target_family = "wasm"))]
  {
    ...
    let block_all =
      crate::pixel_world::world::plugin::should_block_tasks(rendering, async_behavior);
...
pub(crate) fn dispatch_chunk_loads(
  ...
  for world in worlds.iter() {
    for (pos, slot_idx) in world.active_chunks() {
      ...
      io_dispatcher.send(crate::pixel_world::persistence::IoCommand::LoadChunk {
    ...
    loading.tasks.retain(|pos, task| {
      ...
      for mut world in worlds.iter_mut() {
        if let Some(slot_idx) = world.get_slot_index(*pos) {
          ...
          if slot.is_loading() {
            slot.lifecycle = crate::pixel_world::world::slot::ChunkLifecycle::Seeding;
          }
        }
      }
...
pub struct LoadedChunkDataStore {
  ...
  pub store: std::collections::HashMap<
    crate::pixel_world::coords::ChunkPos,
    crate::pixel_world::persistence::LoadedChunk,
  >,
  /// Map from chunk position to loaded body data.
  pub bodies: std::collections::HashMap<
    crate::pixel_world::coords::ChunkPos,
    Vec<crate::pixel_world::persistence::io_worker::BodyLoadData>,
  >,
}
...
impl LoadedChunkDataStore {
  ...
  ) -> Option<crate::pixel_world::persistence::LoadedChunk> {
...
use crate::pixel_world::persistence::io_worker::{IoDispatcher, IoResult};
...
fn handle_chunk_loaded_result(
  ...
  data: Option<crate::pixel_world::persistence::io_worker::ChunkLoadData>,
  bodies: Vec<crate::pixel_world::persistence::io_worker::BodyLoadData>,
) {
  ...
  if let Some(chunk_data) = data {
    let storage_type = match chunk_data.storage_type {
      0 => crate::pixel_world::persistence::format::StorageType::Empty,
      1 => crate::pixel_world::persistence::format::StorageType::Delta,
      _ => crate::pixel_world::persistence::format::StorageType::Full,
    };
