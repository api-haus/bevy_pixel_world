...
use bevy_pixel_world::collision::{
    marching_squares, simplify_polylines, triangulate_polygon, GRID_SIZE,
};
...
fn setup(mut commands: Commands) {
    ...
    commands.spawn((
        ...
        Projection::Orthographic(OrthographicProjection {
            ...
            scaling_mode: ScalingMode::FixedVertical { viewport_height: 35.0 },
...
fn draw_pipeline_stages(mut gizmos: Gizmos) {
    ...
    gizmos.line_2d(Vec2::new(-5.0, -5.0), Vec2::new(-5.0, 30.0), Color::srgba(0.3, 0.3, 0.3, 0.5));
    gizmos.line_2d(Vec2::new(10.0, -5.0), Vec2::new(10.0, 30.0), Color::srgba(0.3, 0.3, 0.3, 0.5));
    gizmos.line_2d(Vec2::new(25.0, -5.0), Vec2::new(25.0, 30.0), Color::srgba(0.3, 0.3, 0.3, 0.5));
    gizmos.line_2d(Vec2::new(40.0, -5.0), Vec2::new(40.0, 30.0), Color::srgba(0.3, 0.3, 0.3, 0.5));
    ...
    for polyline in &simplified {
        ...
        for v in polyline {
            gizmos.circle_2d(*v + Vec2::new(panel2_x, 0.0), 0.4, Color::srgb(0.0, 0.5, 1.0));
        }
    }
    ...
    for polyline in &simplified {
        ...
        for v in polyline {
            gizmos.circle_2d(*v + Vec2::new(panel3_x, 0.0), 0.4, Color::srgb(0.1, 0.6, 0.2));
        }
    }
    ...
    if !PRINTED.swap(true, std::sync::atomic::Ordering::Relaxed) {
        ...
        println!("Raw polylines: {} (vertices: {:?})",
            raw_polylines.len(),
            raw_polylines.iter().map(|p| p.len()).collect::<Vec<_>>()
        );
        println!("Simplified polylines: {} (vertices: {:?})",
            simplified.len(),
        ...
        for (i, polyline) in simplified.iter().enumerate() {
            ...
            let triangles = triangulate_polygon(polyline);
            println!("Polyline {}: {} vertices -> {} triangles", i, polyline.len(), triangles.len());
            for (j, tri) in triangles.iter().enumerate() {
                println!("  Triangle {}: indices ({}, {}, {})", j, tri.0, tri.1, tri.2);
            }
        }
    }
}
