...
///
/// # Arguments
/// * `grid` - A 34x34 binary grid where `true` indicates solid/collision pixels.
///            The grid includes a 1-pixel border around the 32x32 tile.
/// * `tile_origin` - World coordinates of the tile's bottom-left corner.
///
/// # Returns
/// A vector of closed polylines in world coordinates.
pub fn marching_squares(grid: &[[bool; GRID_SIZE]; GRID_SIZE], tile_origin: Vec2) -> Vec<Vec<Vec2>> {
    ...
    for cy in 0..GRID_SIZE - 1 {
        for cx in 0..GRID_SIZE - 1 {
            ...
            for &((x1, y1), (x2, y2)) in EDGE_TABLE[case] {
                ...
                // Cell (cx, cy) corresponds to pixel position (cx-1, cy-1) relative to tile origin
                // because we have a 1-pixel border
...
mod tests {
    ...
    fn test_solid_grid_produces_boundary_contour() {
        ...
        assert!(!contours.is_empty(), "Fully solid grid should produce boundary contours");
    }
    ...
    fn test_single_solid_pixel_produces_contour() {
        ...
        assert_eq!(contours[0].len(), 4, "Single pixel contour should have 4 vertices (diamond shape)");
    }
    ...
    fn test_solid_block_produces_contours() {
        ...
        assert!(total_vertices >= 8, "Solid block contours should have sufficient vertices");
    }
}
