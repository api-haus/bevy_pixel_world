...
/// * `tolerance` - Maximum allowed perpendicular distance from the simplified line.
///
/// # Returns
...
pub fn douglas_peucker(polyline: &[Vec2], tolerance: f32) -> Vec<Vec2> {
    ...
    // half1 ends with the same point that half2 starts with (the second split point)
    // half2 ends with the same point that half1 starts with (the first split point)
    // We need to remove these duplicates to get a clean closed polygon.
...
mod tests {
    ...
    fn test_simplify_reduces_colinear_points() {
        ...
        assert!(simplified.len() < line.len(), "Colinear points should be reduced");
    }
    ...
    fn test_perpendicular_distance() {
        ...
        assert!((dist_sq - 25.0).abs() < 0.001, "Distance should be 5.0 (squared = 25.0)");
    }
}
