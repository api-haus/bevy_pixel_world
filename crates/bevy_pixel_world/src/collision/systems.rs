...
use super::marching::{marching_squares, GRID_SIZE};
...
/// Extracts a 34x34 binary grid for a tile, including 1px border from neighbors.
///
        ...
        for gx in 0..GRID_SIZE {
            ...
            if let Some(pixel) = world.get_pixel(pos) {
                ...
                let is_collision = matches!(
                    material.state,
                    PhysicsState::Solid | PhysicsState::Powder
                );
...
/// System: Dispatches async collision generation tasks for dirty tiles near query points.
