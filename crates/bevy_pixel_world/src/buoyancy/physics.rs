...
/// Applies physics modifications based on submersion state for avian2d.
#[cfg(feature = "avian2d")]
pub fn apply_submersion_physics(
  config: Res<SubmersionPhysicsConfig>,
  mut bodies: Query<(
    &SubmersionState,
    &mut avian2d::prelude::GravityScale,
    &mut avian2d::prelude::LinearDamping,
    &mut avian2d::prelude::AngularDamping,
  )>,
) {
  for (state, mut gravity, mut linear, mut angular) in bodies.iter_mut() {
...
/// Applies physics modifications based on submersion state for rapier2d.
