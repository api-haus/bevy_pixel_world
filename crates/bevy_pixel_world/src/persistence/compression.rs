...
pub fn compute_delta<S: ChunkSeeder>(
    chunk: &Chunk,
    pos: ChunkPos,
    seeder: &S,
) -> Vec<DeltaEntry> {
...
pub fn decode_delta(data: &[u8]) -> Result<Vec<DeltaEntry>, DeltaError> {
    ...
    for i in 0..count {
        ...
        let entry = DeltaEntry::read_from(&raw[offset..])
            .ok_or(DeltaError::InvalidEntry)?;
...
/// Returns whether delta encoding is beneficial for the given modification count.
...
mod tests {
    ...
    fn delta_entry_round_trip() {
        let entry = DeltaEntry::new(
            12345,
            Pixel::new(MaterialId(5), ColorIndex(10)),
        );
