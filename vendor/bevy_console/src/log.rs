...
/// Writer implementation which writes into a buffer resource inside the bevy
/// world
...
/// Creates a tracing layer which writes logs into a buffer resource inside the
/// bevy world This is used by the console plugin to capture logs written by
/// bevy Use [make_filtered_layer] for more customization options.
pub fn make_layer(
  app: &mut App,
...
/// Creates a tracing layer which writes logs into a buffer resource inside the
/// bevy world Uses a custom [EnvFilter] string, allowing for a different subset
/// of log entries to be captured by the console.
