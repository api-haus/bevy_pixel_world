...
pub use crate::console::{
  ...
  ConsoleOpen, ConsoleState, NamedCommand, PrintConsoleLine,
};
pub use crate::log::*;
...
pub enum ConsoleSet {
  ...
  /// Systems running after command systems, which depend on the fact commands
  /// have executed beforehand (the output layer). For example a system which
  /// makes use of [`PrintConsoleLine`] messages should be placed in this set to
  /// be able to receive New lines to print in the same frame
  PostCommands,
}

/// Run condition which does not run any command systems if no command was
/// entered
