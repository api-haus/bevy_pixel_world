...
///
/// Used to capture console commands which implement [`CommandName`],
/// [`CommandArgs`] & [`CommandHelp`]. These can be easily implemented with the
/// [`ConsoleCommand`](bevy_console_derive::ConsoleCommand) derive macro.
///
/// # Example
///
/// ```
...
impl<T> ConsoleCommand<'_, T> {
  ...
  ///
  /// See [`reply_failed!`](crate::reply_failed) for usage with the [`format!`]
  /// syntax.
...
pub struct ConsoleConfiguration {
  ...
  /// for example [vec!["custom", "foo"]], will complete `custom foo` when
  /// typing `custom`
...
pub struct ConsoleCache {
  ...
  /// this probably should operate over references to save memory, but this is
  /// convenient for now
...
pub trait AddConsoleCommand {
  ...
  ///
  /// This registers the console command so it will print with the built-in
...
impl AddConsoleCommand for App {
  fn add_console_command<T: Command, Params>(
    ...
    self
      .add_systems(Startup, sys.in_set(ConsoleSet::Startup))
...
/// Internal console state, including command history.
#[derive(Resource)]
pub struct ConsoleState {
  /// Current input buffer.
  pub buf: String,
  pub(crate) scrollback: Vec<String>,
  /// Command history. Index 0 is always empty (current input buffer).
  /// Index 1+ contains previously executed commands.
...
fn style_ansi_text(str: &str, config: &ConsoleConfiguration) -> LayoutJob {
  ...
  for (str, overrides) in parse_ansi_styled_str(str).into_iter() {
    ...
    for o in overrides {
      match o {
        TextFormattingOverride::Bold => current_style.font_id.size = 16f32, /* no support for
                                                                              * bold font families
                                                                              * in egui TODO:
                                                                              * when egui supports
                                                                              * bold font families,
        ...
        TextFormattingOverride::Dim => {
          // no support for dim font families in egui TODO: when egui supports dim font
          // families, use them here
      ...
      .show(ctx, |ui| {
        ...
        ui.vertical(|ui| {
          ...
          let text_edit = TextEdit::singleline(&mut state.buf)
            ...
            // .lock_focus(true)  // DISABLED: Testing backspace issue
          ...
          if ui.input(|i| i.key_pressed(egui::Key::Tab)) && !cache.predictions_cache.is_empty() {
